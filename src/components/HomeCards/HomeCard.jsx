...
const HomeCard = ({ data, isMatching }) => {
    ...
    return (
        <div className='  flex flex-col mx-auto rounded-md' style={cardStyles}>
            <div className='p-16 flex-grow'>
                <div className="avatar items-center rounded-l-3xl  mb-3 flex flex-col">
                    <div className="w-20   rounded-full ring ring-primary ring-offset-base-100 ring-offset-2">
                        <img src={data?.photoURL} />
                    </div>
                    <p className='text-xl mt-3  p-1'>{data?.name} </p>
                </div>
                <div className='space-y-5 mt-6 flex-grow '>
                    <div className="flex justify-between ">
                        {/* <p className='text-xl flex '>Title : </p> */}
                        <span className=" text-xl font-bold  mx-auto ">{data?.title}</span>
                    </div>
                    <div className="flex justify-between">
                        {/* <p className='text-xl '>Posting Date : </p> */}
                        <span className="text-xl font-bold mx-auto">{data?.dates?.[0]}</span>
                    </div>
                    <div className="flex justify-between">
                        {/* <p className='text-xl '>Application Deadline :</p> */}
                        <span className="text-xl font-bold mx-auto">{data?.dates?.[1]}</span>
                    </div>
                    <div className="flex justify-between">
                        {/* <p className='text-xl '>Salary Range :</p> */}
                        <span className="text-xl font-bold mx-auto">{data?.salary}</span>
                    </div>
